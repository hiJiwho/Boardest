  ...
  Future<void> _handleSave() async {
    ...
    final strokes = _strokes.map((s) {
      return AnnotationStroke(
        ...
        color: s.points.first.color,
        strokeWidth: s.points.first.strokeWidth,
      );
    }).toList();

    final penDoc = PenDocument.fromStrokesMap(
      title: widget.title,
      strokesPages: {widget.currentPage: strokes},
      classroom: widget.classroom,
    );

    if (CloudDriveService.instance.isLoggedIn) {
      await CloudDriveService.instance.savePenDocumentToAdf(
        penDoc,
        originalFileName: '${widget.title}.pen',
        classroom: widget.classroom,
      );
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('판서가 .pen 벡터 규격으로 안전하게 저장되었습니다! 🖊️')),
      );
    }
  }
