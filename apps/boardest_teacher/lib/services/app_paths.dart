...
/// Boardest 교사용 앱 데이터 경로 (%APPDATA%\jiwho.boardest.teacher)
class AppPaths {
  AppPaths._();

  static const String appName = 'jiwho.boardest.teacher';
  static String? _dataRoot;
  static bool _initialized = false;

  /// 앱 기본 최상위 데이터 폴더 (%APPDATA%\jiwho.boardest.teacher)
  ...
  /// 설정 및 자료 저장 폴더 (%APPDATA%\jiwho.boardest.teacher\bst-save)
  static String get bstSaveRootSync {
    return p.join(dataRootSync, 'bst-save');
  }

  /// 판서 저장 폴더 (%APPDATA%\jiwho.boardest.teacher\bst-pen)
  ...
  /// C++ / C# 헬퍼 오버레이 실행 폴더 (%APPDATA%\jiwho.boardest.teacher\Cpp-runner)
  ...
  /// 설정 저장 폴더 위치 (%APPDATA%\jiwho.boardest.teacher\bst-save\config)
  static String get configDir => p.join(bstSaveRootSync, 'config');

  /// 학교 기본 설정 파일 위치 (%APPDATA%\jiwho.boardest.teacher\bst-save\config\school_config.json)
  ...
  static double adaptiveUiScale(BuildContext context, double userScaleFactor) {
    ...
    // High-DPI 모니터에서 UI가 미세하게 축소되지 않도록 최소 1.0 보장
    final adaptive = fit.clamp(1.0, 2.5);
