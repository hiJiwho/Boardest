...
/// Boardest Unified .pen 판서 저장소 서비스
/// 
/// 📌 3대 핵심 실행 및 저장 환경:
...
class AnnotationStorageService {
  ...
  // ─── 클래스 코드 변환 [교실ID] ──────────────────────────────────────────
  /// "1학년 1반" → "[101]", "2학년 3반" → "[203]", 특별실 → "[과학실]", 기본 → "[101]"
  ...
  /// 표준 .pen 파일명/폴더명 생성 규칙 (3가지 경우의 수 100% 적용)
  static String formatPenName({
    ...
    // 1) Teacher 앱으로 실행 (기본값: [Teacher], 특정 반 선택 시: [101] 등)
    if (isTeacherApp) {
      final classPrefix = (classroomId != null &&
              classroomId.trim().isNotEmpty &&
              classroomId != 'teacher' &&
              classroomId != 'Teacher' &&
              classroomId != '교사용' &&
              classroomId != '전체 반 공용 (통합)' &&
              classroomId != '교사용 (Teacher)')
          ? classToCode(classroomId)
          : '[Teacher]';
      if (isWhiteboard) {
  ...
  /// 특정 문서/파일에 대해 실제로 생성된 적이 있는 반(.pen) 목록 조회 (기본값: '교사용 (Teacher)')
  Future<List<String>> getAvailableClassesForDocument(String type, String fileName) async {
    final list = <String>['교사용 (Teacher)'];
    try {
      final clean = fileName.trim().replaceAll(RegExp(r'\.pen$|\.bstpen$|\.pdf$|\.pptx?$'), '');
      if (!kIsWeb) {
        ...
        if (await dir.exists()) {
          ...
          for (final f in files) {
            ...
            if (name.endsWith('.pen') && name.contains(clean)) {
              final match = RegExp(r'^\[(.*?)\]').firstMatch(name);
              if (match != null) {
                final code = match.group(1)!;
                if (code.toLowerCase() != 'teacher') {
                  final label = codeToClass(code);
  ...
  static String codeToClass(String code) {
    final clean = code.replaceAll('[', '').replaceAll(']', '').trim();
    if (clean.toLowerCase() == 'teacher' || clean == '교사용') return '교사용 (Teacher)';
    final digits = clean.replaceAll(RegExp(r'[^\d]'), '');
    if (digits.length >= 2) {
      final g = digits.substring(0, 1);
      final c = int.tryParse(digits.substring(1)) ?? 0;
      final classFormatted = c.toString().padLeft(2, '0');
      return '$g학년 $classFormatted반';
    }
    return clean;
  }
  ...
  /// PDF/PPT/HWP/문서/칠판 판서 저장 (.pen 표준 + Cloud 폴더 자동 동기화)
  Future<void> saveDocumentAnnotations(
    String type, // 'PDF', 'PPT', 'HWP', 'WHITEBOARD', 'WEB', 'TBP', 'CANVA'
    String fileName,
    ...
    try {
      ...
      // 3. Cloud (Google Drive) 저장
      final cloud = CloudDriveService.instance;
      final shouldSaveCloud = isCloud || cloud.isLoggedIn || (fullFilePath != null && fullFilePath.startsWith('cloud:'));

      if (shouldSaveCloud && cloud.isLoggedIn) {
        try {
          await cloud.saveDocumentPenFolderToDrive(
            originalFileName: fileName,
            classCode: isTeacherApp ? '[Teacher]' : classToCode(effectiveClass),
            teacherName: teacherId ?? cloud.userName,
            type: type,
            totalPages: totalPages,
            metadata: metadata,
            penData: penData,
          ...
          debugPrint('[AnnotationStorageService] Cloud pen save error: $e');
        }
      }

      // 4. BstCloudService 실시간 동기화 상태 갱신
      try {
        BstCloudService.instance.saveSyncState(formattedPenName, penData);
      } catch (_) {}

      debugPrint('[AnnotationStorageService] Successfully saved .pen: $formattedPenName ($type)');
    } catch (e) {
  ...
  /// PDF/PPT/HWP/칠판 판서 로드 (.pen 우선, 레거시 .bstpen/.iwb 폴백)
  ...
  Future<void> clearDocumentAnnotations(
    ...
    try {
      final effectiveClass = classroomId ?? className;
      final metaFile = await _resolveBstsaveFile(type, fileName, fullFilePath: fullFilePath, classroomId: effectiveClass, teacherId: teacherId);
      if (metaFile.existsSync()) await metaFile.delete();

      final penFile = await _resolvePenFile(type, fileName, fullFilePath: fullFilePath, classroomId: effectiveClass, teacherId: teacherId);
