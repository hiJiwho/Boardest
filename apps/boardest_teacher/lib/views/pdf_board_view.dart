...
class _PdfBoardViewState extends State<PdfBoardView>
  ...
  String? _pdfFilePath;
  String _selectedClassForPen = '교사용 (Teacher)';
  List<String> _classList = ['교사용 (Teacher)'];
  ...
  Future<void> _loadPdf(String filePath, {Uint8List? bytes}) async {
    ...
    try {
      ...
      if (bytes != null) {
        ...
        if (CloudDriveService.webMemoryFiles.containsKey(filePath)) {
          doc = await PdfDocument.openData(CloudDriveService.webMemoryFiles[filePath]!);
      ...
      final availableClasses = await AnnotationStorageService.instance
          .getAvailableClassesForDocument('PDF', _fileName);

      final loadedAnnotations = await AnnotationStorageService.instance
          .loadDocumentAnnotations(
            'PDF',
            _fileName,
            fullFilePath: filePath,
            className: _selectedClassForPen,
            forcedFile: !kIsWeb && widget.forcedIwbPath != null
                            ...
                            child: Row(
                              children: [
                                ...
                                DropdownButton<String>(
                                  ...
                                  onChanged: (val) async {
                                    if (val != null &&
                                        val != _selectedClassForPen) {
                                      await _saveAllAnnotations();
                                      setState(() {
                                        _selectedClassForPen = val;
                                        _pageAnnotations.clear();
                                        for (final ctrl
                                            in _pageControllers.values) {
                                          ctrl.clear();
                                        }
                                      });
                                      if (_pdfFilePath != null) {
                                        final loaded =
                                            await AnnotationStorageService
                                                .instance
                                                .loadDocumentAnnotations(
                                                  'PDF',
                                                  _fileName,
