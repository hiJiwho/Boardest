...
import 'package:bst_pen/bst_pen.dart' hide ToolMode, ShapeType, TbpInputMode;
...
class _BoardestPenViewState extends State<BoardestPenView>
  ...
  List<int> _selectedStrokeIndices =
      []; // Indices of strokes currently selected by lasso
  Offset? _dragSelectedStart;
  bool _selectEntireStroke = true; // true: 획 선택, false: 부분 선택
  List<Offset> _lastLassoPolygon =
      []; // Keeps copy of the last drawn lasso path
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (contents != null && contents.isNotEmpty) {
          ...
          final totalPages = (iwbData['totalPages'] as num?)?.toInt() ?? 1;
      ...
      final totalPages =
          (iwbData['totalPages'] as num?)?.toInt() ??
          (pageStrokes.keys.isEmpty
              ? 1
              : pageStrokes.keys.reduce((a, b) => a > b ? a : b));
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('판서 파일을 불러오는 중 오류가 발생했습니다: $e')),
        );
      }
    }
  }
  ...
  Future<void> _autoSaveBoard() async {
    try {
      ...
      if (kIsWeb) {
        ...
        debugPrint('[BoardestPenView] Auto-saved whiteboard to web LocalStorage ($key)');
        return;
      }
      ...
      for (int pIndex = 1; pIndex <= _totalPages; pIndex++) {
        final strokes = _pageStrokes[pIndex] ?? [];
        strokesPages[pIndex] = strokes.map<AnnotationStroke>((s) {
      ...
      if (_isBstSaveBoard || (widget.teacher != null && widget.subject != null)) {
        ...
        final existing = await BoardStorageService.instance.loadBoardMetadata(_boardFileBaseName);
        ...
        // Cloud 연동 시 ADF bst-pen 자동 백업
        if (CloudDriveService.instance.isLoggedIn) {
          unawaited(CloudDriveService.instance.savePenDocumentToAdf(
            penDoc,
            originalFileName: '$_boardFileBaseName.pen',
            classroom: widget.teacher,
  ...
  void _checkLassoSelection() {
    ...
    for (int i = 0; i < _strokes.length; i++) {
      ...
      bool hasPointInside = stroke.points.any(
        (p) => _isPointInPolygon(p, _lassoPolygon),
      );
      if (hasPointInside) {
        selected.add(i);
      }
  ...
  List<List<Offset>> _splitPolylineByEraser(
    List<Offset> points,
    Offset center,
    double radius,
  ) {
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () => _goTo(
              _currentPage + 1,
            ), // Triggers auto-add if past final page!
          ),
              ...
              if (_tool == ToolMode.pen) {
                ...
                setState(() {
                  ...
                  _selectedStrokeIndices
                      .clear(); // Clear lasso selection on switch!
                });
              }
            },
            scale,
          ...
          children: [
            ...
            TextButton(
              style: TextButton.styleFrom(
                backgroundColor: !_eraseEntireStroke
                    ? const Color(0xFF00F5D4).withOpacity(0.2)
                    : Colors.transparent,
                foregroundColor: !_eraseEntireStroke
            ...
            Text(
              '${_eraserSize.toStringAsFixed(0)}px',
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            TextButton(
              style: TextButton.styleFrom(
                backgroundColor: _selectEntireStroke
                    ? const Color(0xFF00F5D4).withOpacity(0.2)
                    : Colors.transparent,
                foregroundColor: _selectEntireStroke
                    ? const Color(0xFF00F5D4)
                    : Colors.white70,
                padding: const EdgeInsets.symmetric(
                  horizontal: 10,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // 파일 가져오기 (.pen / .iwb)
            _buildMenuItem(Icons.file_open_rounded, '판서 파일 가져오기 (.pen / .iwb)', () async {
              setState(() => _isMenuOpen = false);
              final result = await fp.FilePicker.pickFiles(
                dialogTitle: '판서 파일 가져오기',
                type: fp.FileType.custom,
                allowedExtensions: ['pen', 'iwb'],
              );
            ...
            // 판서 저장 / 내보내기 (.pen / .iwb / .pdf)
            _buildMenuItem(Icons.save_rounded, '💾 판서 저장/내보내기 (.pen / .iwb / .pdf)', () {
              setState(() => _isMenuOpen = false);
              _saveBoard();
            }),
            _buildMenuItem(
  ...
  Future<void> _saveBoard() async {
    final selectedFormat = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.file_download_outlined, color: Color(0xFF00F5D4), size: 26),
          ...
          children: [
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.draw_rounded, color: Colors.white, size: 26),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('🖊️ .pen (차세대 벡터 규격 - 권장)',
                              style: GoogleFonts.notoSansKr(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                          Text('SVG 벡터 곡선 및 순서 정보 완벽 보존',
                              style: GoogleFonts.notoSansKr(color: Colors.white70, fontSize: 11)),
                        ],
                      ),
                    ),
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.folder_zip_rounded, color: Color(0xFF00F5D4), size: 26),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('📁 .iwb (대화형 칠판 포맷)',
                              style: GoogleFonts.notoSansKr(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                          Text('전자칠판 범용 백업 규격',
                              style: GoogleFonts.notoSansKr(color: Colors.white60, fontSize: 11)),
                        ],
                      ),
                    ),
                  ],
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.picture_as_pdf_rounded, color: Color(0xFFFF6B6B), size: 26),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
    ...
    try {
      if (selectedFormat == 'pen') {
        // Convert to AnnotationStroke map
        final Map<int, List<AnnotationStroke>> strokesPages = {};
        for (int p = 1; p <= _totalPages; p++) {
          final strokes = _pageStrokes[p] ?? [];
          strokesPages[p] = strokes.map<AnnotationStroke>((s) {
            return AnnotationStroke(
              points: s.points,
              color: s.color,
              strokeWidth: s.strokeWidth,
        ...
        if (!kIsWeb && outputFile != null) {
          ...
          if (ok && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('판서가 .pen 벡터 포맷으로 안전하게 저장되었습니다! 🖊️')),
            );
          }
        } else if (kIsWeb && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('판서 파일(.pen)이 성공적으로 다운로드되었습니다! 🖊️')),
          );
        }
        return;
      }

      if (selectedFormat == 'iwb') {
        ...
        final outputFile = await fp.FilePicker.saveFile(
          ...
          fileName: '${_boardFileBaseName.isNotEmpty ? _boardFileBaseName : 'board_backup'}.iwb',
          type: fp.FileType.custom,
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('판서가 IWB 포맷으로 성공적으로 저장되었습니다! 💾')),
          );
        }
        return;
      }
      ...
      if (selectedFormat == 'pdf') {
        ...
        if (!kIsWeb && outputFile != null) {
          ...
          if (ok && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('판서가 성공적으로 저장되었습니다! 📄')),
            );
          }
        } else if (kIsWeb && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('판서 파일이 다운로드되었습니다! 📄')),
          );
        }
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('판서 저장 중 오류가 발생했습니다: $e')),
        );
      }
    }
  }
