...
class _WebsiteBoardViewState extends State<WebsiteBoardView> {
  ...
  void _checkDHashAndSwitchPage() async {
    ...
    final currentHash =
        '${url.replaceAll(RegExp(r'[^\w\.-]'), '_')}_p$_currentPageIndex';
    ...
    if (widget.initialUrl != null && widget.initialUrl!.isNotEmpty) {
      ...
      try {
        ...
        if (CloudDriveService.instance.isLoggedIn) {
          if (CloudDriveService.instance.userEmail != null)
            params['email'] = CloudDriveService.instance.userEmail!;
          if (CloudDriveService.instance.userName != null)
            params['name'] = CloudDriveService.instance.userName!;
        }
  ...
  void _updateWebviewClickThrough() {
    if (!kIsWeb && Platform.isWindows) {
      try {
        ...
        debugPrint(
          '[WebsiteBoardView] Failed to update webview clickthrough: $e',
        );
      }
    }
  }
  ...
  Widget _buildWebViewLayer() {
    if (kIsWeb) {
      final url = _urlController.text.trim().isNotEmpty
          ...
          : defaultSiteUrl;
      return getIframeViewWidget('web-board-iframe-${url.hashCode}', url);
    } else if (!kIsWeb &&
        Platform.isWindows &&
        _isWebviewInitialized &&
        _winWebviewController != null) {
