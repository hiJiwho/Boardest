                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              const Icon(Icons.cloud_sync_rounded, color: Color(0xFF00F5D4), size: 18),
                              const SizedBox(width: 8),
                              Text(
                                'Google Drive ADF 보안 저장소 용량',
                                style: GoogleFonts.notoSansKr(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13,
                          ...
                          const SizedBox(height: 8),
                          FutureBuilder<int>(
                            future: CloudDriveService.instance.calculateAdfStorageUsage(),
                            builder: (context, snapshot) {
                              ...
                              return Row(
                                ...
                                children: [
                                  Text(
                                    '현재 사용량: $mb MB / 500 MB (권장)',
                                    style: GoogleFonts.notoSansKr(color: Colors.white70, fontSize: 12),
                                  ),
                                  ElevatedButton.icon(
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: const Color(0xFFEF4565).withOpacity(0.2),
                                    ...
                                    onPressed: () async {
                                      final ok = await showDialog<bool>(
                                        ...
                                        builder: (ctx) => AlertDialog(
                                          ...
                                          title: const Text('⚠️ ADF 판서 데이터 정리', style: TextStyle(color: Colors.white)),
                                          content: const Text(
                                            'ADF(보안 공간)에 저장된 과거 판서 기록 및 임시 캐시를 모두 비우시겠습니까?\n(일반 수업 교안 자료는 안전하게 유지됩니다.)',
                                            style: TextStyle(color: Colors.white70, height: 1.4),
                                          ),
                                          actions: [
                                            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('취소')),
                                            ElevatedButton(
                                              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFEF4565)),
                                      ...
                                      if (ok == true) {
                                        await CloudDriveService.instance.cleanupAdfStorage();
                                        if (context.mounted) {
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            const SnackBar(content: Text('✅ ADF 판서 데이터 정리가 완료되었습니다.')),
                                          );
                                          setState(() {});
                                        }
                                      }
                                    ...
                                    label: const Text('ADF 판서 정리', style: TextStyle(fontSize: 11)),
                                  ),
                                ],
                              );
                            },
