...
class _BstViewerRouteState extends State<BstViewerRoute> {
  ...
  Future<void> _loadBst() async {
    try {
      ...
      if (widget.bstPath.toLowerCase().endsWith('.bstcanva')) {
        final embedUrl = parsed['embedUrl'] as String? ?? '';
        final title = parsed['title'] as String? ?? 'Canva Presentation';
        if (embedUrl.isEmpty) {
          setState(() {
            _error = '유효하지 않은 Canva 링크입니다.';
            _loading = false;
          });
          return;
        }
        ...
        if (mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) =>
                  CanvaBoardView(embedUrl: embedUrl, title: title),
            ),
          );
        }
        return;
      ...
      final iwbPath = p.join(
        usbPath,
        'bst',
        'JSON',
        '[$className]$fileName.IWB',
      );
