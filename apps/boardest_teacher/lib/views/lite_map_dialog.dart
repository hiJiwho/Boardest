...
class _LiteMapDialogState extends State<LiteMapDialog> {
  ...
  Future<void> _init() async {
    try {
      ...
      if (_settings?.selectedSchool != null) {
        final raw = await _comcigan.fetchTimetableRaw(
  ...
  void _updateTeacherMappingDefaultsForPeriod(int period) {
    ...
    for (final lesson in _timetable!.lessons) {
      if (lesson.weekday == displayDay && lesson.classTime == period) {
        final lessonTeacher = lesson.teacher
            .replaceAll('*', '')
            .trim()
            .toUpperCase();
  ...
  Future<void> _saveMapping({
    ...
    try {
      ...
      final liteSettings =
          (config['lite_settings'] as Map<String, dynamic>?) ?? {};
  ...
  bool _isHomeroomTeacher() {
    ...
    final selectedTeacherSanitized = _settings!.selectedTeacherId
        .replaceAll('*', '')
        .trim()
        .toUpperCase();
  ...
  Widget build(BuildContext context) {
    ...
    final homeroomClassLabel = hasHomeroom
