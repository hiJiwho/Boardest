...
class AuthService {
  ...
  static String buildClassEmail({
    ...
    final classId = '$grade${classNum.toString().padLeft(2, '0')}';
    return '$classId@$sId.boardest.local';
  }
  ...
  Future<void> logout() async {
    try {
      ...
      if (user != null) {
        // 반 계정은 로그아웃 시 Firestore 계정 즉시 소멸
        final url = _userDocUrl(user.email);
        await http.delete(Uri.parse(url)).timeout(const Duration(seconds: 4));
        final settUrl = '$_firestoreBase/user_settings/${_encodeDocId(user.email)}?key=$_apiKey';
        await http.delete(Uri.parse(settUrl)).timeout(const Duration(seconds: 4));
      }
    } catch (_) {}
  ...
  /// 회원 탈퇴 (로그아웃과 완전 통합되어 Firestore 계정 소멸 및 세션 정리)
  ...
  Future<String?> loginOrSignupClass({
    ...
    const password = '!1QAZ@2WSX#3edc\$4rfv%5tgb^6yhn&7ujm*8ik,(9ol.)0p;/';
  ...
  Future<String?> signupWithRawPassword({
    ...
    try {
      ...
      try {
        final authUrl = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=$_apiKey';
        ...
        if (authRes.statusCode == 200) {
          final authData = json.decode(authRes.body) as Map<String, dynamic>;
        ...
        debugPrint('[AuthService] Firebase Auth signUp warning: $err');
      }
      ...
      if (settings != null) {
        fields['settingsJson'] = {'stringValue': json.encode(settings.toJson())};
      }
  ...
  Future<String?> loginWithRawPassword({
    ...
    final url = _userDocUrl(email);
    try {
      // 1. Firebase Authentication 로그인 시도
      try {
        final authUrl = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=$_apiKey';
        final authRes = await http.post(
          Uri.parse(authUrl),
          headers: {'Content-Type': 'application/json'},
          body: json.encode({
        ...
        debugPrint('[AuthService] Firebase Auth signIn warning: $err');
      }
      ...
      // 클라우드 계정에 저장된 환경설정이 있으면 로컬 저장소에 복원
      final settingsStr = (fields['settingsJson'] as Map?)?['stringValue'] as String?;
      if (settingsStr != null && settingsStr.isNotEmpty) {
        try {
          final settingsMap = json.decode(settingsStr) as Map<String, dynamic>;
          final cloudSettings = AppSettings.fromJson(settingsMap);
          if (cloudSettings.selectedSchool != null) {
            await StorageService().saveSettings(cloudSettings);
            debugPrint('[AuthService] Restored AppSettings from cloud account for $email');
          }
        } catch (e) {
          debugPrint('[AuthService] Failed to parse cloud settings: $e');
        }
      }
  ...
  /// Firestore 계정에 환경 설정(AppSettings) 백업 저장
  Future<bool> saveUserSettings(String email, AppSettings settings) async {
    if (email.isEmpty) return false;
    final url = '$_firestoreBase/users/${_encodeDocId(email)}?updateMask.fieldPaths=settingsJson&key=$_apiKey';
    try {
      final body = {
        'fields': {
          'settingsJson': {'stringValue': json.encode(settings.toJson())},
        }
      };
      ...
      if (res.statusCode == 200) {
        debugPrint('[AuthService] Successfully backed up AppSettings to Firestore for $email');
        return true;
      }
    } catch (e) {
      debugPrint('[AuthService] saveUserSettings error: $e');
    }
    return false;
  }

  /// Firestore 계정에서 환경 설정(AppSettings) 불러오기
  Future<AppSettings?> fetchUserSettings(String email) async {
    ...
    try {
      ...
      debugPrint('[AuthService] fetchUserSettings error: $e');
    }
    return null;
  }
