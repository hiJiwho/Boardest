...
class TimeSettings {
  ...
  // 종례: 마지막 교시 종료 후 n분 후 시작 (기본값 10)
  final int? afternoonAssemblyAfterMinutes;
  // 종례: 종례 진행 시간 m분 (기본값 20)
  final int afternoonAssemblyDuration;
  // 조회: 1교시 시작 전 n분 전 시작
  ...
  TimeSettings({
    ...
    this.afternoonAssemblyAfterMinutes = 10,
    this.afternoonAssemblyDuration = 20,
  ...
  factory TimeSettings.fromJson(Map<String, dynamic> json) {
    return TimeSettings(
      ...
      afternoonAssemblyStart:
          json['afternoonAssemblyStart'] as String? ?? "16:10",
      afternoonAssemblyEnd: json['afternoonAssemblyEnd'] as String? ?? "16:30",
      afternoonAssemblyAfterMinutes:
          json['afternoonAssemblyAfterMinutes'] as int? ?? 10,
      afternoonAssemblyDuration:
          json['afternoonAssemblyDuration'] as int? ?? 20,
      morningAssemblyBeforeMinutes:
          json['morningAssemblyBeforeMinutes'] as int?,
    );
  }
}
...
class AppSettings {
  ...
  final int
  specialClassroomType; // 교실 타입 (0: 교실용, 1: 교수학습실, 2: 교사고정특별실, 3: 예약식특별실)
  ...
  double get canvasAspectRatio => is4by3Ratio ? (4.0 / 3.0) : (16.0 / 9.0);

  /// 교실 타입 분류 헬퍼
  bool get specialClassroomMode => specialClassroomType > 0;
  bool get isClassroom => specialClassroomType == 0;
  bool get isLearningLab => specialClassroomType == 1;
  bool get isTeacherFixedRoom => specialClassroomType == 2;
  bool get isReservationRoom => specialClassroomType == 3;
  bool get hasTimetable => specialClassroomType != 1; // 교수학습실만 시간표 없음
  bool get hasTeacherSchedule => specialClassroomType == 2;
  ...
  AppSettings({
    ...
    this.aspectRatio = '16:9',
       ...
       launcherSlots =
           launcherSlots ?? _getDefaultLauncherSlots(selectedSystemApps);
  ...
  static List<LauncherSlot> _getDefaultLauncherSlots(List<SystemApp>? sysApps) {
    final defaults = <LauncherSlot>[
      ...
      LauncherSlot(
        ...
        name: '',
        id: '',
      ), // 빈칸 3 (기존 전체시간표 제거 - 학사달력과 통합됨)
      LauncherSlot(
        type: LauncherSlotType.empty,
        name: '',
        id: '',
      ), // 빈칸 4 (맨 끝 빈칸 추가로 14개 맞춤)
      LauncherSlot(
        type: LauncherSlotType.boardestTool,
        name: '전체앱',
        id: 'app_drawer',
    ...
    for (final key in textbookImages.keys) {
      if (key.contains('_')) {
        ...
        if (parts.length >= 2) {
          ...
          if (grade == keyGrade &&
              (keyStem.contains(stem) || stem.contains(keyStem))) {
  ...
  factory AppSettings.fromJson(Map<String, dynamic> json) {
    final sysApps =
        (json['selectedSystemApps'] as List?)
    ...
    if (loadedSlots != null) {
      ...
      bool hasOldDefaults = loadedSlots.any(
        (slot) =>
            slot.type == LauncherSlotType.systemApp &&
            oldDefaultIds.contains(slot.id),
      );
      if (hasOldDefaults) {
        ...
        if (loadedSlots.length < 14) {
          while (loadedSlots.length < 14) {
            loadedSlots.add(
              LauncherSlot(type: LauncherSlotType.empty, name: '', id: ''),
            );
          }
    ...
    return AppSettings(
      ...
      specialClassroomType: _migrateClassroomType(
          json['specialClassroomType'] as int?,
          json['specialClassroomMode'] as bool?,
      ),
      ...
      selectedTeacherId:
          json['selectedTeacherId'] as String? ??
          (json['selectedTeacher'] as String? ?? ''),
  ...
  AppSettings copyWith({
    ...
    return AppSettings(
      ...
      specialClassroomType:
          specialClassroomType ??
          (specialClassroomMode != null
              ? (specialClassroomMode ? 3 : 0)
              : this.specialClassroomType),
  ...
  /// 구버전 specialClassroomType → 신버전 마이그레이션
  /// 구: 0=일반, 1=전용교사실, 2=과목특수실, 3=미교수특수실
  /// 신: 0=교실용, 1=교수학습실, 2=교사고정특별실, 3=예약식특별실
  static int _migrateClassroomType(int? rawType, bool? legacyBool) {
    if (rawType == null) {
      // specialClassroomType 키가 없으면 레거시 bool 참조
      return (legacyBool ?? false) ? 2 : 0;
    }
    // B 2.5.8 이상이면 이미 신규 값이므로 그대로 반환
    // 구버전 감지: rawType이 이미 저장된 값이므로 구→신 변환은
    // 최초 1회만 실행 (이후 저장 시 신규 값이 기록됨)
    // 안전 범위 클램프
    if (rawType < 0 || rawType > 3) return 0;
    return rawType;
  }
}
