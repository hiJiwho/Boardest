...
class _HwpOverlayViewState extends State<HwpOverlayView> {
  ...
  Future<void> _startNativeOverlay() async {
    ...
    try {
      // 헬퍼 바이너리 탐색 및 Cpp-runner 복사 실행
      final runnerDir = AppPaths.cppRunnerDirSync;
      String exePath = p.join(runnerDir, 'boardest_hwp_overlay.exe');

      if (!File(exePath).existsSync()) {
        final exeDir = File(Platform.resolvedExecutable).parent.path;
        final candidates = [
          p.join(exeDir, 'boardest_hwp_overlay.exe'),
          p.join(Directory.current.path, 'boardest_hwp_overlay.exe'),
          p.join(
      ...
      final metadata = await AnnotationStorageService.instance
          .loadDocumentMetadata(
            'PDF', // HWP shares PDF Board iwb format & metadata category
            _fileName,
          ...
          .listen((line) {
            ...
            if (line.startsWith('LAST_PAGE:')) {
              ...
              if (idx != null) {
                _lastViewedPage = idx;
                unawaited(
                  prefs.setInt('hwp_page_${widget.initialFilePath}', idx),
                );
              }
              ...
              if (idx != null) {
                ...
                if (total != null && idx >= total - 1) {
                  unawaited(
                    prefs.setBool(
                      'hwp_completed_${widget.initialFilePath}',
                      true,
                    ),
                  );
                }
  ...
  void _finalizeSessionAndExit() {
    ...
    unawaited(
      AnnotationStorageService.instance.saveDocumentMetadata('PDF', _fileName, {
