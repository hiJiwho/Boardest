...
class _BoardestPenViewState extends State<BoardestPenView>
  ...
  DrawingStroke _strokeFromMap(Map<String, dynamic> strokeData) {
    try {
      ...
      final points = pointsData.map((pt) {
        final dx = pt['dx'];
        final dy = pt['dy'];
        return Offset(
  ...
  Future<void> _autoSaveBoard() async {
    try {
      ...
      // 1. Google Drive Cloud 실시간 동기화 (토큰이 있는 경우)
      if (BstCloudService.instance.activeToken != null) {
        final cloudPenName = _boardFileBaseName.endsWith('.pen')
            ? _boardFileBaseName
            : '$_boardFileBaseName.pen';
        unawaited(
      ...
      if (_isBstSaveBoard ||
        ...
        final existing = await BoardStorageService.instance.loadBoardMetadata(
          _boardFileBaseName,
        );
        ...
        child: Column(
          ...
          children: [
            ...
            _buildMenuItem(Icons.picture_as_pdf_rounded, 'PDF 파일로 내보내기', () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('판서 칠판이 PDF 문서로 성공적으로 익스포트되었습니다! 📄'),
                ),
              );
              setState(() => _isMenuOpen = false);
            }),
