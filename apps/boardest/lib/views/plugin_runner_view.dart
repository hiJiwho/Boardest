...
class _PluginRunnerViewState extends State<PluginRunnerView> {
  ...
  void _loadManifestAndInit() async {
    ...
    final manifestFile = File(
      p.join(appDir.path, 'plugins', widget.pluginId, 'manifest.json'),
    );
  ...
  void _handleSdkEvent(Map<String, dynamic> data) async {
    ...
    if (type == 'close') {
      ...
      if (token != null && folderId != null) {
        final files = await BstCloudService.instance.fetchDriveFiles(
          folderId,
          token,
        );
        filesList = files
                  ...
                  child: Stack(
                    children: [
                      ...
                      if (_requiresCanvas && _showDrawingToolbar)
                        AnnotationCanvas(controller: _canvasController),
                    ],
                  ),
                ),
