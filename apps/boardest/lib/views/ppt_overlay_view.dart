...
class _PptOverlayViewState extends State<PptOverlayView> {
  ...
  Future<void> _startNativeOverlay() async {
    ...
    try {
      ...
      String exePath = p.join(runnerDir, 'boardest_ppt_overlay.exe');

      if (!File(exePath).existsSync()) {
        final exeDir = File(Platform.resolvedExecutable).parent.path;
        final candidates = [
          p.join(exeDir, 'boardest_ppt_overlay.exe'),
          p.join(Directory.current.path, 'boardest_ppt_overlay.exe'),
          p.join(
          ...
          .listen((line) {
            ...
            if (line.startsWith('LAST_PAGE:')) {
              ...
              if (idx != null) {
                ...
                // PPT별 마지막 페이지 영구 저장
                unawaited(
                  prefs.setInt('ppt_page_${widget.initialFilePath}', idx),
                );
              ...
              if (idx != null) {
                ...
                if (total != null && idx >= total - 1) {
                  unawaited(
                    prefs.setBool(
                      'ppt_completed_${widget.initialFilePath}',
                      true,
                    ),
                  );
                }
  ...
  void _finalizeSessionAndExit() {
    ...
    unawaited(
      AnnotationStorageService.instance.saveDocumentMetadata('PPT', _fileName, {
