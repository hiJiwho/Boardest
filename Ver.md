...
> **현재 시스템 버전**: **v2.9.8.8** (App: v2.9.8.8 / Web: v2.9.8.8)  
> *이 문서는 각 패치 및 메이저 업데이트 시 수행된 핵심 작업 내역을 종합 기록합니다.*

---

## 📌 B 2.9.8.8 (2026-09-04)

### 1. Boardest 대시보드 1:4:1 황금비 & 1:2 수직 분할 레이아웃 완성
- **수평 분할 1 : 4 : 1 (일과표 : 메인 : 런처)**:
  - 오늘의 시간표(`flex: 1`) : 중앙 메인 영역(`flex: 4`) : Bst 런처(`flex: 1`).
- **중앙 메인 영역 수직 분할 1 : 2 (시계 : 수업/급식/광고판)**:
  - 상단(`flex: 1`): 와이드 초대형 시계 카드가 중앙 4폭을 100% 단독 점유하여 폰트 크기와 가독성을 극대화.
  - 하단(`flex: 2`):
    - **일반 모드**: 좌측 지금 수업(`flex: 3`, 가로 배치) : 우측 [급식(`flex: 2`) / 광고판(`flex: 3`)](`flex: 1`).
    - **USB 모드**: 좌측 지금 수업(`flex: 3`) : 우측 [급식(`flex: 2`, 유지!) / USB 탐색기(`flex: 3`)](`flex: 1`).
    - **Cloud 모드**: 급식 제거, 좌측 지금 수업(`flex: 1`, 세로 배치: 상단 교과서, 하단 과목명) : 우측 Cloud 패널(`flex: 3`).
- **우측 런처 도구 7개 통합 & 정리**:
  - 학생연결, 플러그인, Canva, 교과서 제외 (Canva는 Cloud 수업자료에서, 교과서는 Cloud `📖 TBP 교과서` 탭에서 직관적 실행).
  - Bst 도구 일원화: 판서하기, Cloud, 날씨, 학사달력, 앱서랍, 타이머, 설정.
  - Windows: 2열 슬롯(도구 7개 + 시스템 앱 7개), Web: 1열 더블폭 슬롯(도구 7개).

### 2. Teacher Cloud 폴더 뷰 & 백그라운드 동기화 (판서 제외)
- **Google Drive 폴더 브라우징 (📁)**:
  - Cloud 드라이브 패널에 폴더 표시, 폴더 클릭 시 하위 폴더 진입 및 브레드크럼(`← bst-save > [폴더명]`) 상위 이동 지원.
- **EXE 로컬 폴더 자동 백그라운드 동기화**:
  - 설정된 PC 로컬 폴더를 60초 주기로 Drive와 자동 동기화.
  - **단, 판서 파일(`.pen`, `.bstpen`, `.iwb`)은 동기화 대상에서 완전 제외**.
- **외부 파일 플랫폼별 실행 정책**:
  - PDF: 전 플랫폼(Windows, Android, Web) Boardest `PdfBoardView` 판서 뷰어로 실행.
  - PPT, HWP: Windows만 `PptOverlayView`, `HwpOverlayView` 직접 실행. Web 및 Android는 브라우저 다운로드 또는 OS 기본 연결 앱으로 실행.
