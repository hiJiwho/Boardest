...
class _CanvaBoardViewState extends State<CanvaBoardView> {
  ...
  ShapeType _shape = ShapeType.line;
  ...
  static const String _suppressPopupCssJs = '''
    (function() {
      function purgeCookieBanner() {
        ...
        elements.forEach(el => {
          if (el.innerText && (el.innerText.includes('쿠키에 동의') || el.innerText.includes('cookie') || el.innerText.includes('Cookie'))) {
            const btns = el.querySelectorAll('button');
            btns.forEach(btn => {
              if (btn.innerText && (btn.innerText.includes('모든 쿠키 허용') || btn.innerText.includes('동의') || btn.innerText.includes('Allow all'))) {
    ...
    final jsCode = '''
      (function() {
        ...
        for (var i = 0; i < count; i++) {
          window.dispatchEvent(new KeyboardEvent('keydown', { key: '$jsKey', code: '$jsKey', keyCode: $keyCode, which: $keyCode, bubbles: true }));
        }
      })();
    ''';
