...
bool FlutterWindow::OnCreate() {
  ...
  method_channel_->SetMethodCallHandler(
      [this](const flutter::MethodCall<flutter::EncodableValue>& call,
        ...
        if (call.method_name() == "triggerSleep") {
          ...
          special_classroom_mode_ = enabled;
          
          // Restore normal mode (borderless maximized to rcWork, shows taskbar, non-topmost) always to prevent squishing
          ...
          if (mode == 1 || mode == 2) {
            ...
            if (special_classroom_mode_) {
              // Stay in Special Classroom Mode (right 20% NOTOPMOST)
              LONG style = original_style_;
              style &= ~(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU);
              SetWindowLong(hwnd, GWL_STYLE, style);
  ...
  flutter_controller_->engine()->SetNextFrameCallback([&]() {
    ...
    special_classroom_mode_ = start_special;
